...
--1.	Đưa ra thông tin gồm mã số, họ tên và tên khoa của tất cả các giảng viên
	...
	FROM TBLGiangVien GV,TBLKhoa K
...
--2.	Đưa ra thông tin gồm mã số, họ tên và tên khoa của các giảng viên của khoa ‘DIA LY va QLTN’
	...
	SELECT MAGV,Hotengv,TENKHOA 
	FROM TBLGiangVien GV,TBLKhoa K
	WHERE GV.Makhoa=K.Makhoa AND Tenkhoa='Dia ly va QLTN'

--3.	Cho biết số sinh viên của khoa ‘CONG NGHE SINH HOC’

	SELECT COUNT(MASV) AS 'SỐ SINH VIÊN KHOA CONG NGHE SINH HOC'
	FROM TBLSinhVien SV,TBLKhoa K
	WHERE SV.Makhoa=K.Makhoa AND Tenkhoa='CONG NGHE SINH HOC'

--4.	Đưa ra danh sách gồm mã số, họ tên và tuổi của các sinh viên khoa ‘TOAN’

	SELECT MASV, Hotensv,(YEAR(GETDATE()) - Namsinh) AS 'TUỔI'
	FROM TBLSinhVien SV,TBLKhoa K
	WHERE SV.Makhoa=K.Makhoa AND Tenkhoa='TOAN'

--5.	Cho biết số giảng viên của khoa ‘CONG NGHE SINH HOC’

	SELECT COUNT(MAGV) AS 'SỐ GIẢNG VIÊN KHOA CONG NGHE SINH HOC'
	FROM TBLGiangVien GV,TBLKhoa K
	WHERE GV.Makhoa=K.Makhoa AND Tenkhoa='CONG NGHE SINH HOC'
...
--7.	Đưa ra mã khoa, tên khoa và số giảng viên của mỗi khoa
	...
	SELECT K.Makhoa,Tenkhoa,TENKHOA,COUNT(MAGV) AS 'SỐ GIẢNG VIÊN'
	FROM TBLGiangVien GV,TBLKhoa K
	WHERE GV.Makhoa=K.Makhoa 
...
--8.	Cho biết số điện thoại của khoa mà sinh viên có tên ‘Le van son’ đang theo học
	...
	SELECT Dienthoai
	FROM TBLKhoa K, TBLSinhVien SV
	WHERE K.Makhoa=SV.Makhoa AND Hotensv='Le van son'
